...
const EditTaskModal = ({ task, members = [], onSave, onClose }) => {
  ...
  const handleSubmit = async e => {
    ...
    const { error: err } = await onSave(task.id, form);
    setLoading(false);
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <motion.div className="modal-box" initial={{ opacity: 0, y: 16, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0 }} onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div className="form-group">
            ...
            <input className="form-input" value={form.title} onChange={e => set('title', e.target.value)} maxLength={200} required autoFocus />
          </div>
          ...
          <div className="form-group">
            ...
            <input className="form-input" value={form.blocker_note} onChange={e => set('blocker_note', e.target.value)} placeholder="Any blockers?" />
          </div>
          ...
          <div className="modal-footer" style={{ paddingTop: 12, marginTop: 0 }}>
            ...
            <button type="submit" className="btn btn-primary" disabled={loading}>
              {loading ? <Loader2 size={14} style={{ animation: 'spin 0.7s linear infinite' }} /> : 'Save Changes'}
            </button>
          </div>
        </form>
